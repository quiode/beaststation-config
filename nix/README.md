...
The zfs datasets with actual data (so not the boot partition, rpool, etc.) are encrypted using a passphrase. On boot, the datasets have to be decrypted and mounted using `unlock`. The passphrase also could be placed under `/config/secrets/passphrase.txt` but one would still have to unlock the ssd at-least once.
